...
output_root = fullfile(pwd, 'generated_heat_data', 'varying_baseline');
...
% Probability that an input keeps its stepped level as the updated baseline
p_drift = 1.0;
...
%% ==========================
% Input constraints and stepped schedule with baseline drift
%% ==========================
u_nom = [0; 0; 0; 0];
u_min = [0; 0; 0; 0];
u_max = [5; 5; 5; 5];
...
cfgs = make_scheduled_pulses_rest(T_end, order, durations, u_nom, ...
    abs_change_min, abs_change_max, ...
    u_min, u_max, ...
    p_drift, rest_min, rest_max);

u1_fun = @(t) eval_step_drift_input(t, cfgs.u1, u_nom(1), u_min(1), u_max(1));
u2_fun = @(t) eval_step_drift_input(t, cfgs.u2, u_nom(2), u_min(2), u_max(2));
u3_fun = @(t) eval_step_drift_input(t, cfgs.u3, u_nom(3), u_min(3), u_max(3));
u4_fun = @(t) eval_step_drift_input(t, cfgs.u4, u_nom(4), u_min(4), u_max(4));
...
filename = fullfile(output_dir, sprintf('heat_system_varying_baseline_%s_%s_%s_%s_Tend%g_Ts%g.csv', ...
    C_tag, duration_tag, H_mode_tag, noise_tag, T_end, Ts));
writetable(T, filename);

fprintf('CSV file written: %s\n', filename);

meta_filename = fullfile(output_dir, sprintf('heat_system_varying_baseline_%s_%s_%s_%s_Tend%g_Ts%g_meta.csv', ...
    C_tag, duration_tag, H_mode_tag, noise_tag, T_end, Ts));
...
function cfgs = make_scheduled_pulses_rest(T_end, order, durations, u_nom, ...
    abs_change_min, abs_change_max, ...
    u_min, u_max, ...
    p_drift, rest_min, rest_max)
n = numel(order);
...
while tcur < T_end
    ...
    lo = u_min(idx);
    hi = u_max(idx);
    base = clip_scalar(baselines(idx), lo, hi);
    level = propose_level_absolute(base, abs_change_min.(nm), abs_change_max.(nm), lo, hi);
    ...
    cfgs.(nm).baseline(end+1,1) = base;
    cfgs.(nm).settled(end+1,1)  = base;

    if rand < p_drift
        baselines(idx) = level;
        cfgs.(nm).settled(end,1) = level;
    end
...
function u = eval_step_drift_input(t, cfg, u_nom, u_min, u_max)
u = u_nom;
