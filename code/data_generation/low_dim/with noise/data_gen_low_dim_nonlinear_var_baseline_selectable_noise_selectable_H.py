...
from low_dim_noise_h_utils import (
    ...
    PULSE_HOURS,
    build_metadata_rows,
    build_varying_baseline_schedule,
    format_token,
    hidden_profile_rows,
    make_h_profile,
    noise_tag,
...
def main() -> None:
    ...
    data_dir = repo_dir / "Process data" / "low dim" / "with noise and 5'hs" / "var baseline"
    tag = experiment_tag()

    model = LowDimNonlinearBenchmarkModel(state_noise_std=STATE_NOISE_STD)
    input_schedule, events = build_varying_baseline_schedule(seed=INPUT_TARGET_SEED)
    ...
    for h_mode in H_MODES:
        ...
        for noise_mode in NOISE_MODES:
            ...
            output_path = data_dir / f"low_dim_nonlinear_var_baseline_{tag}_{h_mode_tag}_{noise_mode_tag}.csv"
            schedule_path = data_dir / f"low_dim_nonlinear_var_baseline_schedule_{tag}_{h_mode_tag}_{noise_mode_tag}.csv"
            metadata_path = data_dir / f"low_dim_nonlinear_var_baseline_{tag}_{h_mode_tag}_{noise_mode_tag}_meta.csv"
            hidden_path = data_dir / f"low_dim_nonlinear_var_baseline_{tag}_{h_mode_tag}_{noise_mode_tag}_hidden.csv"
